...
class JsonApp
{
    ...
    public function handleRequest()
    {
        if (isset($_GET['page'])) {
            $page = $_GET['page'];
            $this->routeRequest($page);
        } else {
            $this->homepage($this->conn);
        }
    }

    private function routeRequest($page)
    {
        switch ($page) {
            case 'migrate':
                $this->startDbMigrate($this->conn);
                break;
            case 'register':
                $this->registerPage();
                break;
            case 'register_handle':
                $this->userRegisterHandle($this->conn);
                break;
            case 'login':
                $this->loginPage();
                break;
            case 'login_handle':
                $this->userLoginHandle($this->conn);
                break;
            case 'logout_handle':
            ...
            case 'add':
                $this->addJsonPage();
                break;
            case 'add_json_handle':
                $this->addJsonHandle($this->conn);
                break;
            case 'edit':
                if (isset($_GET['id'])) {
                    $id = $_GET['id'];
                    $this->editJsonPage($this->conn,$id);
                } else {
                    $this->nf404Page(true);
                }
                break;
            case 'json':
                if (isset($_GET['id'])) {
                    $id = $_GET['id'];
                    $this->getJson($this->conn,$id);
                } else {
                    $this->nf404Page(true);
                }
                break;
            case 'edit_json_handle':
                $this->updateJsonHandle($this->conn);
                break;
            case 'delete':
                $this->deleteJsonHandle($this->conn);
                break;
            default:
                $this->homepage($this->conn);
                break;
...
?>
    ...
    private function homepage($conn)
    {
        session_start();
        if (isset($_SESSION['user_id']) && isset($_SESSION['username'])) {
            $this->bootstrap_head('Home');
            $this->navbar();
    ...
    private function addJsonHandle($conn)
    {
    ...
    private function basicFormControl($formLabel, $type, $name, $id, $isValueRequired, $value)
    {
        ?>
        <div class="mb-3">
    ...
    private function updateJsonHandle($conn)
    {
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            $id = $_POST['id'];
            $name = $_POST['name'];
    ...
    function editJsonPage($conn, $id)
{

    $this->bootstrap_head('Edit Fish Data Page');
    $this->navbar();
                    ...
                    <div class="container p-4">
                        <form action="?page=edit_json_handle" method="post">
                            <div class="container">
                                ...
                                <?php
                                $this->basicFormControl('Name', 'text', 'name', 'name', true, $row["name"]);
    ...
    private function bootstrap_foot()
    {
    ?>
    ...
    private function bootstrap_head($pagename)
    {
    ?>
        <!doctype html>
        <html lang="en">
    ...
    private function getJson($conn, $id)
    {
...
{
    $this->bootstrap_head('Add JSON Data Page');
    $this->navbar();
    ?>
    <div class="container p-4">
        <form action="?page=add_json_handle" method="post" enctype="multipart/form-data">
            <div class="container">
                ...
                $this->basicFormControl('Name', 'text', 'name', 'name', false, null);
                ?>
                <div class="mb-3">
                    <label class="form-label">Json</label>
    ...
    private function nf404page($isRequiredNavbar)
    {
        $this->bootstrap_head('404 Not Found');
    ...
    private function userRegisterHandle($conn)
    {
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    {
        $this->bootstrap_head('Login Page');
        ?>
            <div class="container d-flex justify-content-center align-items-center vh-100">
                ...
                <form action="?page=login_handle" method="post" class="mx-auto" style="width: 500px;">
                    ...
                    $this->basicFormControl('Email Address', 'email', 'username', 'username', false, null);
                    $this->basicFormControl('Password', 'password', 'password', 'password', false, null);
                    ?>
                    <button type="submit" class="btn btn-dark">Submit</button>
                </form>
            </div>
    ...
    function deleteJsonHandle($conn)
{
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        $id = $_POST['id'];
    ...
    {
        $this->bootstrap_head('Register Page');
        session_start();
        ?>
            <div class="container d-flex justify-content-center align-items-center vh-100">
                <form action="?page=register_handle" method="post" class="mx-auto" style="width: 500px;">
                    ...
                    $this->basicFormControl('Username', 'text', 'fullname', 'fullname', false, null);
                    $this->basicFormControl('Email Address', 'email', 'username', 'username', false, null);
                    $this->basicFormControl('Password', 'password', 'password', 'password', false, null);
                    ?>
                    <button type="submit" class="btn btn-dark">Submit</button>
...
$jsonApp = new JsonApp($dbConfig);
$jsonApp->handleRequest();
